...
Local IDs `D1…D45`. These become GitHub issues on the plan lap that follows the
...
### D9 — perf: the episode loop at ≥100× real time, asserted in the gate
...
vectorised with NumPy, writing the episode log. Add a gate assertion that a
6-vehicle, 256×256, 400-tick episode completes at ≥100× the simulated wall
clock on a laptop.
...
- [ ] The speed assertion is in the gate and currently passes
...
- [ ] The digest is small enough that a 4-minute episode log stays under 50 MB
...
- [ ] `--truth` colour appears nowhere but the truth marker
...
- [ ] Combined fixture size stays under 25 MB
...
## Notes
Run **D44 (S1)** first — the CPU ceiling is real and six instances was our
untested cut. `SPEC.md` §10. Depends on D6, D19.
...
## Notes
`SPEC.md` §9 item 4. Depends on D21.
...
## What
**Timeboxed to one hour.** Start `sim_vehicle.py --count 4 --auto-sysid
--mcast`, connect `pymavlink`, confirm GUIDED `SET_POSITION_TARGET_GLOBAL_INT`
moves a vehicle. Record CPU and memory. Report the instance ceiling.
...
## Notes
`SPEC.md` §10. No production code. Depends on D3.
...
## Blocked by
**Open questions Q1 and Q6** (`SPEC.md` §11): do they provide the harness, and
...
**Milestone:** M3 · **Demo beat:** — · **Size:** S
**Labels:** `docs` `P2` `area:transport` `loop-ok`
...
- [ ] Each possible answer to Q1/Q6 names the files that would change
...
|---|---|---|
| M0 | 3 | D1–D3 |
| M1 | 24 | D4–D20, D22, D24, D34, D35, D36, D41, D45 |
| M2 | 8 | D21, D23, D26, D28, D29, D31, D37, D44 |
| M3 | 5 | D25, D30, D32, D38, D43 |
| M4 | 2 | D39, D40 |
| M5 | 3 | D27, D33, D42 |
...
`needs-decision`: **2** — D31 (Q2) and D32 (Q1/Q6). Everything else is
`loop-ok`.
